...
def load_lt_metadata(lt_path):
    ...
    for book in data.values():
        ...
        if 'ddc' in book and 'code' in book['ddc']:
            ...
            ddc_code = str(ddc_code).strip()

        if not ddc_code: continue
...
def balance_and_execute(node, threshold, dry_run, library_root):
    """
    ...
    for key in active_subfolders:
        ...
        balance_and_execute(child_node, threshold, dry_run, library_root)
...
def main():
    ...
    for child_key, child_node in virtual_root.children.items():
        balance_and_execute(child_node, args.threshold, args.dry_run, root_dir)
